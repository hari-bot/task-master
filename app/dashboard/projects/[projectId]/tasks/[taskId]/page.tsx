...
export default function TaskPage({
  ...
  useEffect(() => {
    const fetchTask = async () => {
      try {
        const taskData = await getTask(params.projectId, params.taskId)
        setTask(taskData)
        setFormData({
    ...
    fetchTask()
  }, [params.projectId, params.taskId, router, toast])
  ...
  if (!task) {
    return (
      <div className="flex h-[calc(100vh-4rem)] flex-col items-center justify-center">
        ...
        <Button onClick={() => router.push(`/dashboard/projects/${params.projectId}`)} className="mt-4">
          Go back to project
        </Button>
      </div>
    )
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">Update task details or change its status</p>
      </div>
      <Card>
        <form onSubmit={handleSubmit}>
          <CardHeader>
            <CardTitle>Task Details</CardTitle>
            <CardDescription>Make changes to your task information</CardDescription>
          </CardHeader>
